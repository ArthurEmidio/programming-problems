...
int main()
{
    ...
    for (int i = 0; i < N; i++) {
        ...
        // the diameter of the minimum bounding circle of an obtuse triangle is its max side
        double minRadius = t.classifyAngles() == Triangle::OBTUSE ? max(max(a, b), c)/2 : t.circumradius();
                
        vector<int> fit;
        for (int j = 0; j < M; j++) {
            double radius = holes[j];
            if (radius >= minRadius) fit.push_back(j + 1);
        }
        
        printf("Peg %c will ", 'A' + i);
        if (!fit.empty()) {
            printf("fit into hole(s):");
            for (int hole : fit) printf(" %d", hole);
        } else {
