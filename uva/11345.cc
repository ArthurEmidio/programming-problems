...
int main()
{
    ...
    while (n--) {
        ...
        int xll, yll, xur, yur;
        cin >> xll >> yll>> xur >> yur;
        Rectangle inter(Point(xll, yll), Point(xur, yur));
        
        bool hasIntersection = xll <= xur && yll <= yur;
        for (int i = 1; i < m; i++) {
            cin >> xll >> yll>> xur >> yur;
            Rectangle r(Point(xll, yll), Point(xur, yur));
            if (hasIntersection) hasIntersection = r.intersection(inter, inter) && xll <= xur && yll <= yur;
        }
        
        printf("Case %d: %.0f\n", tc++, hasIntersection ? inter.area() : 0);
    }
